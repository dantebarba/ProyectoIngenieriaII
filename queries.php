...
function q_removeEditorial($idEditorial) {
    $query="DELETE FROM editoriales WHERE idEditorial='$idEditorial'";
    mysql_query($query) or die(mysql_error());
}

function q_removeCategoria($etiqueta) {
    $query="DELETE FROM etiquetas WHERE idEtiquetas='$etiqueta'";
    mysql_query($query) or die(mysql_error());
}
